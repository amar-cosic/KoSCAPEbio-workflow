...
if (a>2) {
  # Read the data
  main_pre_abs <- read.csv("sup_pressence_absence.csv", header = TRUE, sep = ";", row.names = 1)
  ...
  species_order <- c(  "QIIME level 7 - Ko",
                       "K_oxytoca_kdb" ,
                       "K_grimontii_kdb", 
                       "K_shared_GPS_kdb", 
                       "K_shared_MP_kdb",
  ...
  heatmap_pressence
  ggsave("sup_pressence_absence.pdf", plot = heatmap_pressence, width = 25, height = 8, unit = "in")
}
# Load required libraries
library(ggplot2)
library(reshape2)
...
# Read the data
main_abundance <- read.csv("sup_abundance.csv", header = TRUE, sep = ";", row.names = 1)
...
max_value <- 100
print(min(abundance_long$Value[abundance_long$Value > 0]))
# Adjust the values parameter to increase the visibility of low values
color_gradient <- scale_fill_gradientn(
  colors = colors,
  values = scales::rescale(c(0, 0.004675592, 100)),  # Adjust this line to change color distribution
...
heatmap_plot <- ggplot(abundance_long, aes(x = Marker, y = Species, fill = Value)) +
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
        axis.text.y = element_text(color = "black"))
heatmap_plot
ggsave("sup_abundance.pdf", plot = heatmap_plot, width = 25, height = 8, unit = "in")
